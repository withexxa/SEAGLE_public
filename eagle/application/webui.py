...
def bot(history, temperature, top_p, use_EaInfer, highlight_EaInfer, use_Exxa_optimize, session_state,):
    import eagle.model.cnets as cnets
    cnets.ENABLE_SEAGLE = use_Exxa_optimize
    if not history:
    ...
    if use_EaInfer:
        ...
        for output_ids in model.ea_generate(input_ids, temperature=temperature, top_p=top_p,
            ...
            total_ids+=1
            if start_time is None and total_ids==1:
...
parser.add_argument(
    "--no-eagle3", action="store_true", help=" Not use EAGLE-3", default=False
)
...
with gr.Blocks(css=custom_css) as demo:
    ...
    with gr.Row():
        with gr.Column():
            ...
            use_Exxa_optimize = gr.Checkbox(label="Exxa optimize", value=True)
            highlight_EaInfer = gr.Checkbox(label="Highlight the tokens generated by EAGLE-3", value=True)
        temperature = gr.Slider(minimum=0.0, maximum=1.0, step=0.01, label="temperature", value=0.0)
    ...
    msg = gr.Textbox(label="Your input", max_lines=5)
    with gr.Row():
    ...
    enter_event=msg.submit(user, [msg, chatbot,gs], [msg, chatbot,gs], queue=True).then(
        bot, [chatbot, temperature, top_p, use_EaInfer, highlight_EaInfer, use_Exxa_optimize, gs], [chatbot,speed_box,compression_box,gs]
    )
